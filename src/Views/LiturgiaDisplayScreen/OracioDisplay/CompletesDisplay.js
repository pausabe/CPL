...
export default class CompletesDisplay extends Component {
  ...
  render() {
    if(this.props.liturgicProps.LITURGIA.completes !== null){
      ...
      return (
        <View>
          {this.props.liturgicProps.LT === GLOBAL.Q_TRIDU && this.props.variables.date.getDay() === 6?
            <View>
          ...
          {Platform.OS === 'android' ? <Text>{"\n"}</Text> : <Text />}
          <Text selectable={true} style={this.styles.black}>{COMPLETES.actePen}</Text>
          {Platform.OS === 'android' ? <Text>{"\n"}</Text> : <Text />}
          <HR/>
          {Platform.OS === 'android' ? <Text>{"\n"}</Text> : <Text />}
      ...
      console.log("this.props.liturgicProps.LITURGIA.completes doesen't exists");
      return null;
    }
  }
